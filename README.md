...
- **The guest sees a realistic Windows filesystem.** When a bottle's drive is materialized, WIE seeds the default folder skeleton — `C:\Windows` (+ `System32`), `C:\Program Files` / `Program Files (x86)`, `C:\ProgramData`, `C:\Users\Public` (+ `Documents`), the `C:\Users\WIE` profile (Documents, Desktop, Downloads, Pictures, Music, Videos, `AppData\Roaming` / `Local` / `LocalLow`, Favorites, Links, Saved Games, Searches, Templates, Contacts) and `C:\Temp`. Path-returning APIs — `GetWindowsDirectory`, `GetSystemDirectory`, `GetTempPath`, `SHGetFolderPath` — point at these seeded directories, which exist on the host under `{root}/drive_c/…`. Re-seeding is idempotent.
