...
**Real console 7-Zip (`7za`):** WIE runs the **Windows PE64** standalone console from 7-Zip Extra (not macOS `7za`, not GUI). The PE is **not** committed (`real_exes/` is gitignored) — download once, then create/list/extract `.7z` under JIT or iced. See [`docs/7zip.md`](docs/7zip.md).

**Interactive games:** WIE runs terminal games like 2048 and Snake compiled with mingw-w64. See [`docs/2048.md`](docs/2048.md) and [`docs/snake.md`](docs/snake.md).
