...
| Item | Status |
| --- | --- |
| Feasibility pass: map x19–x28 = rax…r15 fixed order, `x14` rflags carrier; enumerate spill sites in `jit/lower/*` + `engine.rs` trampoline | 🟡 **done 2026-09-05** — spill sites enumerated: dispatcher entry mass-copy (`pipeline.rs run_compiled`), block-entry live-GPR/rflags loads (`lower/mod.rs compile_block`), **per-chain-edge `writeback_gprs` + successor reload (`lower/emit.rs emit_chain_or_exit` — the hot tax)**, block-exit stores, dispatcher exit writeback; Rust-side trampoline twins (`mark_dirty`/`chain_tail`). ABI constraint: literal x19–x28 residency is inexpressible via Cranelift under the aarch64 C ABI (17 results > register window, no custom call-conv) → staged plan: tail-call chaining (blocked, see next row) → hand-written prologue stub (trampolines.rs pattern) for the reg map → SSA rflags (details in ADR-0002 status) |
| Direct block→block chaining (`b`/`br`, regs live) with dispatcher fallback (miss/SMC/fake-VA) | 🔴 **slice 1 reverted 2026-09-05** — tail-call chain hops (`return_call`/`return_call_indirect` in `emit_chain_or_exit`, f415d6b) never compiled: Cranelift 0.133 permits `return_call` only under `CallConv::Tail` (non-ABI-stable), and the block signature must stay on the host default to remain callable from Rust as `extern "C"`, so the verifier rejected every chain edge ("calling convention `…` does not support tail calls") and each such block fell back to the interpreter (35 test failures). Default flipped back to off — shipped hop is the nested-call + `MAX_CHAIN_DEPTH` guard shape; `WIE_JIT_TAILCHAIN=1` force-enables for experiments. Tail chaining stays blocked until the prologue stub (row above) lets blocks adopt a tail-call-capable convention or Cranelift lifts the restriction. regs-live hand-off still awaits the prologue-stub ABI; dispatcher fallback/IC/chain-table untouched |
