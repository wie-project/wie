...
impl WieApp {
    ...
    fn mouse_target(&self, handle: &GuestHandle) -> (u64, u16, u16) {
        ...
        handle
            .window_at(x, y)
            .map_or((self.hwnd.unwrap_or(0), 0, 0), |(hwnd, rx, ry)| {
                (hwnd, rx as u16, ry as u16)
            })
    }
...
impl ApplicationHandler<WieEvent> for WieApp {
    ...
    fn window_event(
        ...
        // Handle events that need early processing before the hwnd guard.
        match &event {
            WindowEvent::CloseRequested => {
                if let Some(handle) = self.handle.as_ref()
                    && let Some(hwnd) = self.hwnd
                {
                    handle.post_message(hwnd, 0x0010, 0, 0); // WM_CLOSE
                }
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                if let Some(ref handle) = self.handle {
                    // Fresh-lookup hwnd: use cached OR ask the guest directly.
                    let hwnd = self.hwnd.or_else(|| handle.first_guest_window_handle());
                    if let Some(hwnd) = hwnd {
                        // Cache it so subsequent key events find it fast.
                        self.hwnd = Some(hwnd);
        ...
        match event {
            WindowEvent::RedrawRequested => {
                if let (Some(surface), Some(window)) = (&mut self.surface, self.window.as_ref()) {
                    let frame = handle.take_frame(hwnd);
                    if let Some(frame) = frame {
                        ...
                        if self.last_presented_generation == Some(generation)
                            && self.last_presented_size == Some((dst_w, dst_h))
                        {
                            tracing::debug!(
                                target: "wiegui",
                                generation,
                        ...
                        let nzw =
                            std::num::NonZeroU32::new(dst_w).unwrap_or(std::num::NonZeroU32::MIN);
                        let nzh =
                            std::num::NonZeroU32::new(dst_h).unwrap_or(std::num::NonZeroU32::MIN);
                        let _ = surface.resize(nzw, nzh);
                        // B9(d): time copy ③ (copy_from_slice / stretch_nearest
                        // + softbuffer present) — only when frame timing is on.
                        ...
                        if let Ok(mut buf) = surface.buffer_mut() {
                            if src_w == dst_w && src_h == dst_h {
                                // Full copy only. softbuffer's AppKit backend
                                // (cg.rs) allocates a fresh zeroed buffer on
                                // every `buffer_mut()` call — the previous
                                // frame's pixels are never retained — so a
                                // region-only copy would black out everything
                                // outside the region (visible as a black flash
                                // after resize and as element-shaped bands on
                                // partial control repaints). The frame's
                                // `region` field remains for guest-side
                                // bookkeeping and tests, but the presented
                                // buffer is always fully rewritten.
                        ...
                        self.last_presented_generation = Some(generation);
                        self.last_presented_size = Some((dst_w, dst_h));
                    }
                }
            }
            ...
            WindowEvent::Moved(position) => {
                ...
                handle.post_message(hwnd, input::WM_MOVE, 0, lparam);
            }
            ...
            WindowEvent::Focused(true) => {
                handle.post_message(hwnd, input::WM_SETFOCUS, 0, 0);
            }
            WindowEvent::Focused(false) => {
                handle.post_message(hwnd, input::WM_KILLFOCUS, 0, 0);
            }
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if let Some(start) = self.last_resize
            && start.elapsed() >= Duration::from_millis(RESIZE_SETTLE_MS)
        {
            ...
            if let Some((w, h)) = self.pending_size.take() {
                ...
                // close/quit handling).
                if self.last_sent_size != Some((w, h)) {
                    if let (Some(handle), Some(hwnd)) = (self.handle.as_ref(), self.hwnd) {
                        ...
                        handle.resize_window(hwnd, w, h);
                        let lparam = input::make_lparam(w as u16, h as u16);
                        handle.post_message(hwnd, input::WM_SIZE, 0, lparam);
                        handle.post_message(hwnd, input::WM_PAINT, 0, 0);
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: WieEvent) {
        match event {
            WieEvent::Frame { published_at } => {
                ...
                // Always try to set hwnd in case the resumed() handler created
                // the window before the guest published its first frame.
                if self.hwnd.is_none()
                    && let Some(hwnd) = self
                        .handle
                        .as_ref()
                        .and_then(|h| h.first_guest_window_handle())
                {
                    self.hwnd = Some(hwnd);
                }
                if self.window.is_none() {
                        ...
                        .and_then(|h| h.first_guest_window_info())
                    {
                        ...
                        if let Ok(window) = event_loop.create_window(attrs) {
                            ...
                            let surface =
                                ctx.and_then(|c| softbuffer::Surface::new(&c, window.clone()).ok());
            ...
            WieEvent::MenuEvent(menu_event) => {
                ...
                if let Some(handle) = self.handle.as_ref()
                    && let Some(hwnd) = self.hwnd.or_else(|| handle.first_guest_window_handle())
                {
                    self.hwnd = Some(hwnd);
