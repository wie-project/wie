...
use crate::helpers::{
    D3D9_RESTING_FRAME_HASH, FrameWatcher, frame_hash, gui_suite_serialize, micro_exe,
};
...
fn gui_d3d9_commit_thread_frame_matches_legacy_hash() {
    ...
    // The resting frame is transient under commit mode: the committer
    // publishes asynchronously and later frames overwrite the latest-wins
    // slot, so the inter-iteration polls below can miss its whole lifetime
    // on a loaded host. A background watcher samples the channel every 1 ms
    // and latches the first match (see [`FrameWatcher`]; `take_frame` clones
    // the slot, so watcher and test never steal from each other).
    let watcher = {
